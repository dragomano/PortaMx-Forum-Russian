...
$helptxt['loginHistoryDays'] = 'Количество дней для хранения истории входа в систему при отслеживании профиля пользователя. Значение по умолчанию — 7 дней.';
...
$helptxt['minimize_files'] = 'При включении этой опции все CSS и JS файлы, которые загружаются каждый раз для каждого пользователя, будут уменьшены в размере и помещены в один файл, для уменьшения количества запросов к серверу. Эти файлы будут пересоздаваться заново каждый день. Если необходимо пересоздать их прямо сейчас, просто очистите кэш форума.';
